...
#include "../Interface/OperatingSystem.h"
...
typedef void (*REI_LogFn)(REI_LogType, const char*, const char*);
...
typedef enum REI_ResourceState
{
    ...
    REI_RESOURCE_STATE_RESOLVE_DEST = 0x400,
    REI_RESOURCE_STATE_RESOLVE_SOURCE = 0x800,
...
typedef enum REI_DescriptorType
{
    ...
    REI_DESCRIPTOR_TYPE_VERTEX_BUFFER = (REI_DESCRIPTOR_TYPE_UNIFORM_BUFFER << 1),
    ...
    REI_DESCRIPTOR_TYPE_INPUT_ATTACHMENT = (REI_DESCRIPTOR_TYPE_RENDER_TARGET_DEPTH_SLICES << 1),
...
{
    REI_SEMANTIC_UNDEFINED = 0,
    REI_SEMANTIC_POSITION,
    REI_SEMANTIC_NORMAL,
    REI_SEMANTIC_COLOR,
    ...
    REI_SEMANTIC_TEXCOORD8,
    REI_SEMANTIC_TEXCOORD9,
} REI_ShaderSemantic;
...
typedef enum REI_DescriptorSetIndex
{
    REI_DESCRIPTOR_SET_INDEX_0,
    REI_DESCRIPTOR_SET_INDEX_1,
    REI_DESCRIPTOR_SET_INDEX_2,
    REI_DESCRIPTOR_SET_INDEX_3,
    REI_DESCRIPTOR_SET_INDEX_4,
    REI_DESCRIPTOR_SET_INDEX_5,
    REI_DESCRIPTOR_SET_INDEX_6,
    REI_DESCRIPTOR_SET_INDEX_7,
    REI_DESCRIPTOR_SET_COUNT,
} REI_DescriptorSetIndex;
...
typedef struct REI_DescriptorInfo   REI_DescriptorInfo;
typedef struct REI_DescriptorSet    REI_DescriptorSet;
...
typedef struct REI_ShaderSource
{
    uint8_t* pByteCode;
...
typedef struct REI_BlendStateDesc
{
    ...
    REI_BlendStateTargets renderTargetMask;
...
typedef struct REI_DepthStateDesc
{
    ...
    REI_CompareMode depthCmpFunc = REI_CMP_LEQUAL;
    bool            stencilTestEnable;
    uint8_t         stencilReadMask;
    uint8_t         stencilWriteMask;
    REI_CompareMode stencilFrontFunc = REI_CMP_ALWAYS;
    REI_StencilOp   stencilFrontFail;
    REI_StencilOp   depthFrontFail;
    REI_StencilOp   stencilFrontPass;
    REI_CompareMode stencilBackFunc = REI_CMP_ALWAYS;
    REI_StencilOp   stencilBackFail;
    REI_StencilOp   depthBackFail;
    REI_StencilOp   stencilBackPass;
} REI_DepthStateDesc;

typedef struct REI_RasterizerStateDesc
{
    REI_CullMode  cullMode;
    float         depthBiasConstantFactor;
    float         depthBiasSlopeFactor;
    REI_FillMode  fillMode;
    bool          multiSample;
    bool          scissor;
    REI_FrontFace frontFace;
} REI_RasterizerStateDesc;

typedef struct REI_VertexAttrib
{
    REI_ShaderSemantic   semantic;
    uint32_t             semanticNameLength;
    char                 semanticName[REI_MAX_SEMANTIC_NAME_LENGTH];
    REI_Format           format;
    uint32_t             binding;
    uint32_t             location;
    uint32_t             offset;
    REI_VertexAttribRate rate;
} REI_VertexAttrib;

typedef struct REI_VertexLayout
{
    uint32_t         attribCount;
    REI_VertexAttrib attribs[REI_MAX_VERTEX_ATTRIBS];
} REI_VertexLayout;
...
typedef struct REI_DeviceCapabilities
{
    ...
    bool                ROVsSupported;
    bool                partialUpdateConstantBufferSupported;
...
{
    REI_FilterType  minFilter;
    REI_FilterType  magFilter;
    REI_MipmapMode  mipmapMode;
    REI_AddressMode addressU;
    REI_AddressMode addressV;
    REI_AddressMode addressW;
    float           mipLodBias;
    float           maxAnisotropy;
    REI_CompareMode compareFunc;
} REI_SamplerDesc;
...
typedef struct REI_DescriptorSetDesc
{
    ...
    uint32_t               maxSets;
} REI_DescriptorSetDesc;

typedef struct REI_RootSignatureDesc
{
    REI_Shader**  ppShaders;
...
{
    uint32_t         stages;
    REI_ShaderSource vert;
    REI_ShaderSource frag;
    REI_ShaderSource geom;
    REI_ShaderSource hull;
    REI_ShaderSource domain;
    REI_ShaderSource comp;
} REI_ShaderDesc;
...
typedef struct REI_GraphicsPipelineDesc
{
    ...
    REI_RootSignature*       pRootSignature;
    REI_VertexLayout*        pVertexLayout;
    ...
    REI_PrimitiveTopology    primitiveTopo;
...
void REI_freeMemoryStats(REI_Renderer* pRenderer, char* stats);
...
void REI_addShader(REI_Renderer* pRenderer, const REI_ShaderDesc* p_desc, REI_Shader** p_shader_program);
void REI_removeShader(REI_Renderer* pRenderer, REI_Shader* p_shader_program);
